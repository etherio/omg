...
export default {
    ...
    async onSubmit(event) {
      ...
      this.axios({
        ...
        .then(() => {
          ...
          this.axios
            .delete(`${server.review}/${this.id}`, {
              headers: { "X-Access-Token": this.$root.user.token },
            })
            .then(() => this.$router.push({ path: "/reviews" }));
        })
        .catch((err) => {
          console.error(err);
  ...
  beforeMount() {
    ...
    if (id) {
      ...
      this.axios
        .get(`${server.review}/${id}`, {
          headers: { "X-Access-Token": this.$root.user.token },
        })
        .then(({ data }) => {
          this.select.name = data.message[0];
          this.select.price = data.message[2];
          this.select.description = data.message[1];
